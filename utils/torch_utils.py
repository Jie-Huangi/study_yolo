...
def smart_DDP(model):
    """Initializes DistributedDataParallel (DDP) for models training, respecting torch version constraints."""
...
def reshape_classifier_output(model, n=1000):
    """Reshapes last layer of models to match class count 'n', supporting Classify, Linear, Sequential types."""
...
def select_device(device="", batch_size=0, newline=True):
    """Selects computing device (CPU, CUDA GPU, MPS) for YOLOv5 models deployment, logging device info."""
...
def is_parallel(model):
    """Checks if the models is using Data Parallelism (DP) or Distributed Data Parallelism (DDP)."""
    return type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)


def de_parallel(model):
    """Returns a single-GPU models by removing Data Parallelism (DP) or Distributed Data Parallelism (DDP) if applied."""
...
def find_modules(model, mclass=nn.Conv2d):
    """Finds and returns list of layer indices in `models.module_list` matching the specified `mclass`."""
    return [i for i, m in enumerate(model.module_list) if isinstance(m, mclass)]


def sparsity(model):
    """Calculates and returns the global sparsity of a models as the ratio of zero-valued parameters to total
    parameters.
    """
    a, b = 0, 0
...
def prune(model, amount=0.3):
    """Prunes Conv2d layers in a models to a specified sparsity using L1 unstructured pruning."""
...
def model_info(model, verbose=False, imgsz=640):
    """
    Prints models summary including layers, parameters, gradients, and FLOPs; imgsz may be int or list.
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        """Initializes EMA with models parameters, decay rate, tau for decay adjustment, and update count; sets models to
        evaluation mode.
        """
    ...
    def update(self, model):
        """Updates the Exponential Moving Average (EMA) parameters based on the current models's parameters."""
        self.updates += 1
    ...
    def update_attr(self, model, include=(), exclude=("process_group", "reducer")):
        """Updates EMA attributes by copying specified attributes from models to EMA, excluding certain attributes by
        default.
        """
