...
def train(hyp, opt, device, callbacks):
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        LOGGER.warning(
            "WARNING ⚠️ DP not recommended, use torch.distributed.runs for best DDP Multi-GPU results.\n"
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp["anchor_t"], imgsz=imgsz)  # runs AutoAnchor
...
def run(**kwargs):
    ...
    Example: import train; train.runs(data='coco128.yaml', imgsz=320, weights='yolov5m.pt')
    """
    opt = parse_opt(True)
