...
class BaseModel(nn.Module):
    ...
    def forward(self, x, profile=False, visualize=False):
        """Executes a single-scale inference or training pass on the YOLOv5 base models, with options for profiling and
        visualization.
        """
        return self._forward_once(x, profile, visualize)  # single-scale inference, train

    def _forward_once(self, x, profile=False, visualize=False, embed=None):
        """Performs a forward pass on the YOLOv5 models, enabling profiling and feature visualization options."""
    ...
    def fuse(self):
        """Fuses Conv2d() and BatchNorm2d() layers in the models to improve inference speed."""
    ...
    def info(self, verbose=False, img_size=640):
        """Prints models information given verbosity and image size, e.g., `info(verbose=True, img_size=640)`."""
        model_info(self, verbose, img_size)

    def _apply(self, fn):
        """Applies transformations like to(), cpu(), cuda(), half() to models tensors excluding parameters or registered
        buffers.
        """
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg="yolov5s-seg.yaml", ch=3, nc=None, anchors=None):
        """Initializes a YOLOv5 segmentation models with configurable params: cfg (str) for configuration, ch (int) for channels, nc (int) for num classes, anchors (list)."""
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg=None, model=None, nc=1000, cutoff=10):
        """Initializes YOLOv5 models with config file `cfg`, input channels `ch`, number of classes `nc`, and `cuttoff`
        index.
        """
        super().__init__()
    ...
    def _from_detection_model(self, model, nc=1000, cutoff=10):
        """Creates a classification models from a YOLOv5 detection models, slicing at `cutoff` and adding a classification
        layer.
        """
    ...
    def _from_yaml(self, cfg):
        """Creates a YOLOv5 classification models from a specified *.yaml configuration file."""
        self.model = None


def parse_model(d, ch):
    """Parses a YOLOv5 models from a dict `d`, configuring layers based on input channels `ch` and models architecture."""
