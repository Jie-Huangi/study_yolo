...
class TFBottleneckCSP(keras.layers.Layer):
    ...
    def call(self, inputs):
        """Processes input through the models layers, concatenates, normalizes, activates, and reduces the output
        dimensions.
        """
...
class TFSPPF(keras.layers.Layer):
    ...
    def call(self, inputs):
        """Executes the models's forward pass, concatenating input features with three max-pooled versions before final
        convolution.
        """
        x = self.cv1(inputs)
        y1 = self.m(x)
...
class TFDetect(keras.layers.Layer):
    ...
    def call(self, inputs):
        """Performs forward pass through the models layers to predict object bounding boxes and classifications."""
        z = []  # inference output
        x = []
        for i in range(self.nl):
...
class TFProto(keras.layers.Layer):
    ...
    def call(self, inputs):
        """Performs forward pass through the models, applying convolutions and upscaling on input tensor."""
...
def parse_model(d, ch, model, imgsz):
    """Parses a models definition dict `d` to create YOLOv5 models layers, including dynamic channel adjustments."""
...
def parse_opt():
    """Parses and returns command-line options for models inference, including weights path, image size, batch size, and
    dynamic batching.
    """
...
def main(opt):
    """Executes the YOLOv5 models runs function with parsed command line options."""
    run(**vars(opt))
